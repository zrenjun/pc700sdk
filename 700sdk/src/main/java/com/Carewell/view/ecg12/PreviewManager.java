...
public class PreviewManager {
    ...
    public synchronized short[][] getCurrentScrrenDrawData(short[][] ecgDataArrayAll, int ecgImageWidth, float length, LeadSpeedType leadSpeedType) {
        ...
        if (ecgDataArrayAll[0].length <= needData) {
            ...
            refreshRthData(needData / (float) SAMPLE_RATE * speed, isFirst, ecgDataArrayAll,speed);
            isFirst=false;
    ...
    private void refreshRthData(float length, boolean isFirst, short[][] data,float speed) {
        ...
        if (!isFirst&&(mScreenShowCount==data[0].length||(count >= 0 && firstIndex_rth >=data[0].length-mScreenShowCount) || (count <= 0 && firstIndex_rth <= 0))) {
            return;
        }
