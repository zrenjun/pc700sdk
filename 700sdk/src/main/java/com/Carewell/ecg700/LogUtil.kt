...
//            try {
                var log = File("${Environment.getExternalStorageDirectory().path}/zrj")
                log.listFiles()?.forEach { it.delete() }
//            val dir = log.listFiles()
//            if (dir != null && (dir.size > 20 || log.length() > 500 * 1024 * 1024)) {//5*4 5天  500M
//                //文件修改日期：递增
...
//            }
                ...
                log = File("${context.getExternalFilesDir(null)?.path}")
                val dir = log.listFiles()
