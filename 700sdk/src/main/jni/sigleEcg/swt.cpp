...
void SWT::SamplesToSwdSwa(double * samples, int sampleCnt)
{
    ...
    //the process of swt
    for (i = 0; i < SWT_FLOOR_NUM; i++)		// Panjie: С���ֽ�Ĳ��������㷨����3��С���ֽ⣬���ֽ�ΪD1��D2��D3��A3
    {
//        exponent2 = (int) pow(2, i);
        exponent2 = MyPow(2, i);
        exponent2Multi8 = FILTER_INI_LEN * exponent2;		// Panjie�� ÿһ�����ʱ���˲����ĳ���
...
//    int senarios;
    ...
    for (i = 0; i < SWT_FLOOR_NUM; i++)
    {
        for (j = 0; j < sampleCnt; j++)
        {
            absX = fabs(swd[i][j]);		// Panjie: ȡϸ��ϵ������ֵ
            arrayForOrder[j] = absX;
        }

        sort(arrayForOrder.begin(), arrayForOrder.end());		// Panjie��sort���������ֱ��Ӧ������������׵�ַ��β��ַ

        if(sampleCnt % 2 == 0)
        {
            lamda = (arrayForOrder[sampleCnt / 2-1] + arrayForOrder[sampleCnt / 2]) / 2;		// Panjie: ȡ��ֵ
        }
        else
        {
...
//        senarios = GetSenarios(x1, x2, x3);
        ...
        {
            absX = fabs(swd[i][j]);
            if (-1 != DoubleCompares(absX, lamda) )		// Panjie: lamdaΪ��ֵ����absX���ڵ���lamdaʱ����absX
            {
                continue;
            }
            else
...
void SWT::SwdSwaToSamples(double * samples, int sampleCnt)
{
    ...
    //the process of iswt  swt����任

	for (i = 0; i < sampleCnt; i++)
	{
		samples[i] = swa[SWT_FLOOR_NUM - 1][i];		// Panjie: ȡ������Ľ���ϵ��
	}
...
void SWT::EcgSwt(double * samples, int sampleCnt)
{
    ...
    if (sampleCnt > SWT_MAX_SAMPLE_CNT)		// Panjie: һ�η�������ֻ����256
    {
        return;
    }
...
void SWT::AnalyzeSwt(deque <double> & samples)
{
    ...
    if (samples.size() != SWT_MAX_SAMPLE_CNT)		// Panjie : �����������ж�
    {
        return;
    }

    double * samplesPtr = new double[samples.size()];		// Panjie�������ڴ�ռ�
